...
the upstream text when bumping a provider CLI. Any app-server error whose
message happens to embed `authentication required` for a non-account reason
(a proxy `407 Proxy Authentication Required` surfaced in the message is the
plausible case) would classify as unauthenticated; the marker was chosen
because upstream exposes no typed signal, and live QA with a signed-out
Codex is the check.
